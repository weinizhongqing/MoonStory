...
import com.ping.night.story.databinding.NsMyFavoritePageBinding
...
class NsFavActivity : NsAdPage() {

    companion object {
        fun startPage(context: Context) {
            val intent = Intent(context, NsFavActivity::class.java)
            context.startActivity(intent)
        }
    }


    private val binding by lazy { NsMyFavoritePageBinding.inflate(layoutInflater) }

    override val currentPageNativeAd: String?
        get() = NsPosition.AD_FUNC_N

    override val currentPageNativeView: CustomNativeAdView?
        get() = binding.favoritePageAd
    ...
    private fun queryDownloadedPager(): Flow<PagingData<LocalVideoInfo>> {
        return Pager(
            ...
            NsDBHelper.instance.localVideoDao().getAllLikePaging()
        }.flow
    }
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding.headPage.pageTiele.text = getString(R.string.videos)
        binding.favoritePageRv.layoutManager = LinearLayoutManager(this)
        binding.favoritePageRv.adapter = mVideoAdapter

        mVideoAdapter.addLoadStateListener {
            when (it.refresh) {
                is LoadState.NotLoading -> {
                    if (mVideoAdapter.itemCount > 0) {
                        binding.favoritePageMap.isVisible = false
                        binding.favoritePageRv.isVisible = true

                    } else {
                        binding.favoritePageMap.isVisible = true
                        binding.favoritePageRv.isVisible = false
                    }
                }
